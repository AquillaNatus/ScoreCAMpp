...
def denormalize_image(normalized_tensor, mean= [0.485, 0.456, 0.406], std= [0.229, 0.224, 0.225]):
    """
    ...
    if not isinstance(mean, (list, tuple)) or not isinstance(std, (list, tuple)):
        raise ValueError("Mean and std should be provided as lists or tuples.")
    ...
    denormalized_tensor = normalized_tensor * std + mean
...
def visualize_(img, cam,filename):
    """
...
def save_tensor(img,count):
    img = img.detach().cpu()
    img = img.squeeze()
    plt.imshow(img)
    path = 'activation_'+str(count)+'.png'
    plt.imsave(path, img)

def save_threshold(img,count):
    img = img.detach().cpu()
    img = img.squeeze()
    plt.imshow(img)
    path = 'after_tanh'+str(count)+'.png'
    plt.imsave(path, img)

def save_after_multiplication(img,count,org):
    img = img.detach().cpu() + org.squeeze().cpu()
    img = img.squeeze()
    img = (img - img.min())/(img.max()-img.min())
    img = img.permute(1,2,0).numpy()
    plt.imshow(img)
    path = 'after_multiplication'+str(count)+'.png'
    plt.imsave(path, img)

def save_final_output(img):
    img = img.detach().cpu()
    img = img.squeeze()
    plt.imshow(img)
    path = 'Final.png'
...
def reverse_normalize(x, mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]):
    ...
    return x
def visualize_(img, cam,filename=None):
    ...
    cam = cam.detach()
    cam = ff.interpolate(cam, size=(H, W), mode='bilinear', align_corners=False)
    cam = 255 * cam.squeeze().cpu().numpy()  # Convert to numpy array
    ...
    result = result.div(result.max())  # Normalize within the [0, 1] range
    result = torch.squeeze(result)
    result = result.permute(1,2,0)
    plt.axis('off')
    plt.imshow(result)
